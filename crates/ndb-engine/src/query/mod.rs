...
fn execute_hyperedge_pattern(
    ...
    for row in rows {
        ...
        // via type cluster. Per-edge type membership is an O(1) reverse-map
        // probe (see `candidate_hyperedges`), so no per-call set cache.
        let candidates = candidate_hyperedges(engine, type_id, role_bindings, &row);
...
/// Candidate hyperedges for a hyperedge pattern.
///
/// When a role term resolves to a concrete entity (the join case), seed
/// from that entity's adjacency list and keep only edges clustered under
/// `type_id`. The type check is an O(1) reverse-map probe per adjacency
/// edge (`engine.hyperedge_has_type`) — no per-query materialisation of
/// the whole type bucket. The earlier implementation built a
/// `HashSet<HyperedgeId>` of every edge of the type once per query (45k
/// inserts on the bench's two_pattern_join just to membership-test ~49
/// short adjacency lists); the reverse-map probe makes the per-edge cost
/// O(1) and the per-query cost O(Σ adjacency_degree).
///
/// Correctness is unchanged: the type filter only narrows *candidates*;
/// the real type + MVCC-visibility checks happen downstream in
/// `hyperedge_pattern_step` via `hyperedge_at`.
fn candidate_hyperedges(
    ...
    let ty = TypeId::new(type_id);
    for rb in role_bindings {
        if let Some(eid) = role_term_to_entity(&rb.term, row) {
            let mut adj = engine.hyperedges_for_entity(eid);
            adj.retain(|h| engine.hyperedge_has_type(*h, ty));
            return adj;
        }
    }
    // Fallback: every hyperedge of this type.
    engine.hyperedges_by_type(ty)
}
