...
2. `npm i @n-dimension-database-ndb/client` → a ~10-line TS program performs `commit` + `query` against the server, working in **Node and a browser/Deno** runtime.
3. `npx @n-dimension-database-ndb/mcp --path ./db` → an MCP client (Claude/Cursor/Codex) connects, lists tools, and runs a hyperedge commit + a time-travel read.
...
- Define a **semver policy** spanning four surfaces, each versioned independently and stated explicitly:
  ...
  - **SDK** (`@n-dimension-database-ndb/client` npm version)
...
### 3. `@n-dimension-database-ndb/client` — TypeScript SDK
...
- **npm name:** `@n-dimension-database-ndb/client`. **Fallback** if the `@ndb` scope cannot be claimed: unscoped `ndb-client`.

### 4. `@n-dimension-database-ndb/mcp` — agent packaging
- npm package wrapping the existing `ndb-mcp-server` binary so `npx @n-dimension-database-ndb/mcp --path ./db` launches it with no Rust toolchain on the user's machine.
- The wrapper resolves the correct prebuilt binary for the host platform via **per-platform `optionalDependencies`** (the esbuild/swc pattern: `@n-dimension-database-ndb/mcp-linux-x64`, `@n-dimension-database-ndb/mcp-linux-arm64`, `@n-dimension-database-ndb/mcp-darwin-arm64`, each shipping one binary; npm installs only the matching one), then `exec`s it, passing through args/stdio. Chosen over a `postinstall` download-from-release because it works offline, behind corporate proxies, and with `npm ci` lockfile integrity.
- **npm name:** `@n-dimension-database-ndb/mcp`. **Fallback:** unscoped `ndb-mcp`.
...
- **npm publish:** `@n-dimension-database-ndb/client` and `@n-dimension-database-ndb/mcp` published from the same tag workflow.
...
- Rewrite `docs/QUICKSTART.md` into two ≤5-minute paths:
  - **Human developer:** `docker run` the server → `npm i @n-dimension-database-ndb/client` → a hello-world TS snippet (commit + query) → pointer to a small example app under `examples/ts-app/`.
  - **AI agent:** `npx @n-dimension-database-ndb/mcp --path ./db` → connect Claude/Cursor/Codex (config snippet) → one hyperedge round-trip showing the distinctive model.
...
| `@n-dimension-database-ndb/client` | Integration suite vs a spawned `ndb-server`; type checks |
| `@n-dimension-database-ndb/mcp` | `tools/list` + hyperedge commit + as-of read smoke test |
...
- npm names: `@n-dimension-database-ndb/client`, `@n-dimension-database-ndb/mcp` (fallback unscoped if scope unavailable).
